...
class Network():
    ...
    def run(self, input):
        ...
        # Flatten the last layer to 0-1
        for i in range(len(current_layer)):
            current_layer[i] = current_layer[i]
        return current_layer
    
    def activation(self, input):
        '''Returns the input modified by the activation function'''
...
class Species():
    ...
    def random_parameters(self):
        ...
        num_hidden = np.random.randint(10)
        layer_size = np.random.randint(10)+1
    ...
    def breed(self, parents, parameters, num_children):
        ...
        total_fitness = max(total_fitness, parents[0].fitness)
        for i in range(num_children):
            ...
            while len(p) < 2:
                if np.random.random() < ((parents[j].fitness+1e-3)/total_fitness)**2:
                    p.append(j)
                j = (j+1)%len(parents)
            ...
            parameters.insert(i*2,[c_hidden, c_layer, c_weights])
        return parameters
...
class SexualSpecies():
    def __init__(self, input_size, output_size, num_hidden, width,
        weight_mut_rate = 0.1):
        self.input_size = input_size
        self.output_size = output_size
    ...
    def add_member(self,key):
        self.members[key] = Network([self.input_size]+\
            [self.width]*self.num_hidden+[self.output_size])
    
    def remove_member(self,key):
        del self.members[key]
    
    def breed(self, p1, p2, new_key):
        self.add_member(new_key)
        p1_w = self.members[p1].weights
        p2_w = self.members[p2].weights
        weights = self.members[new_key].weights
        for i in range(len(weights)):
            for j in range(len(weights[i])):
                for k in range(len(weights[i][j])):
                    if np.random.random() > self.weight_mut_rate:
                        weights[i][j][k] = p1_w[i][j][k] + \
                            np.random.random()*(p2_w[i][j][k]-p1_w[i][j][k])
                    else:
                        weights[i][j][k] = 2*np.random.random()*p1_w[i][j][k]